...
const LoginForm: React.FC<LoginFormProps> = ({switchToSignUp}) => {
  ...
  return (
    <div className="h-[22vh] flex items-center justify-center px-4 ">
      <div className="max-w-md  w-full border border-indigo-200 rounded-2xl shadow-xl p-6  bg-white">
        ...
        <form onSubmit={handleSubmit(submit)} className="flex flex-col gap-3">
          ...
          <center onClick={()=> router.push("/forget-password")}
           className="text-indigo-600 hover:underline cursor-pointer">Forget Password?</center>
        </form>
      </div>
    </div>
  );
